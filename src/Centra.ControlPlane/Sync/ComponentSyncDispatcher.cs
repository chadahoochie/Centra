...
public sealed class ComponentSyncDispatcher : IComponentSyncDispatcher
{
    ...
    private readonly ConcurrentDictionary<string, Channel<ResilienceSyncEventDto>> _resilienceSubscribers = new();

    public int SubscriberCount => _subscribers.Count;
    public int ResilienceSubscriberCount => _resilienceSubscribers.Count;
    ...
    public ValueTask BroadcastResilienceUpdateAsync(ResilienceSyncEventDto syncEvent, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(syncEvent);

        foreach (var channel in _resilienceSubscribers.Values)
        {
            channel.Writer.TryWrite(syncEvent);
        }
    ...
    public async IAsyncEnumerable<ResilienceSyncEventDto> SubscribeResilienceAsync(
        string appId,
        string instanceId,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            _resilienceSubscribers.TryRemove(subscriberKey, out _);
            channel.Writer.TryComplete();
        }
    }
}
