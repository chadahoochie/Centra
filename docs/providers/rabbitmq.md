...
- A dead-letter route is mandatory: `SubscribeAsync` throws when neither `deadLetterTopic` nor an `x-dead-letter-exchange` custom argument is supplied, because a budget-exhausted message would otherwise be discarded and queue arguments cannot be changed after declare.
