...
13. **Consumer-Enforced Redelivery Budget**: Never rely on a broker delivery limit (e.g. RabbitMQ's `x-delivery-limit`) to bound an `EventHandlingResult.Retry` loop - brokers do not count application-initiated requeues, so the loop runs unbounded. The RabbitMQ driver is the only consume path that enforces its own attempt budget today; it applies backoff, dead-letters on exhaustion, and refuses a subscription that has no dead-letter route to exhaust into. Every other driver must reject `PubSubSubscribeOptions.MaxRetryAttempts` / `RetryInitialBackoff` / `RetryMaxBackoff` via `RedeliveryBudgetOptionsGuard` rather than silently ignoring them, until it implements the budget. See [`docs/building-blocks/pubsub.md`](docs/building-blocks/pubsub.md#-bounded-redelivery-budget).
