...
13. **Consumer-Enforced Redelivery Budget**: Never rely on a broker delivery limit (e.g. RabbitMQ's `x-delivery-limit`) to bound an `EventHandlingResult.Retry` loop - brokers do not count application-initiated requeues, so the loop runs unbounded. The RabbitMQ driver is the only consume path that enforces its own attempt budget today, and the budget is opt-in: with `MaxRetryAttempts` unset (or 0) a subscription keeps plain unbounded nack-requeue and its queue is declared with no dead-letter arguments. A subscription that does set a positive budget gets backoff, dead-letters on exhaustion, and is refused at subscribe time if it has no dead-letter route to exhaust into. Every other driver must reject `PubSubSubscribeOptions.MaxRetryAttempts` / `RetryInitialBackoff` / `RetryMaxBackoff` via `RedeliveryBudgetOptionsGuard` rather than silently ignoring them, until it implements the budget. See [`docs/building-blocks/pubsub.md`](docs/building-blocks/pubsub.md#-bounded-redelivery-budget).
